...
GLInjectInput::~GLInjectInput() {
    ...
    if (m_thread.joinable()) {
        blog(LOG_INFO, "[GLInjectInput::~GLInjectInput] %s", "Stopping input thread...");
        m_should_stop = true;
        m_thread.join();
    }
...
bool GLInjectInput::SwitchStream(SharedData* lock, const SSRVideoStream& stream) {
    try {
        ...
        blog(LOG_ERROR, "[GLInjectInput::SwitchStream] %s", "Could not read stream, this usually means that the stream was already gone.");
        return false;
    }
    return true;
}
...
void GLInjectInput::InputThread() {
    try {
        ...
        blog(LOG_INFO, "[GLInjectInput::InputThread] %s", "Input thread started.");
        ...
        blog(LOG_INFO, "[GLInjectInput::InputThread] %s", "Input thread stopped.");

    } catch (const std::exception& e) {
        m_error_occurred = true;
        blog(LOG_ERROR, "[GLInjectInput::InputThread] %s: %s", "Exception in input thread", e.what());
    } catch (...) {
        m_error_occurred = true;
        blog(LOG_ERROR, "[GLInjectInput::InputThread] %s", "Unknown exception in input thread.");
    }
}