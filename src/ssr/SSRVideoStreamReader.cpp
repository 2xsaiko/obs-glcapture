...
void SSRVideoStreamReader::Init() {
    ...
    blog(LOG_INFO, "[SSRVideoStreamReader::Init] %s", "Created video stream reader.");

    // open main file
    m_fd_main = open(m_filename_main.c_str(), O_RDWR | O_CLOEXEC);
    if (m_fd_main == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamReader::Init] %s", "Error: Can't open video stream file!");
    ...
    if (ftruncate(m_fd_main, m_mmap_size_main) == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamReader::Init] %s", "Error: Can't resize video stream file!");
        throw SSRStreamException();
    }
    ...
    if (m_mmap_ptr_main == MAP_FAILED) {
        blog(LOG_ERROR, "[SSRVideoStreamReader::Init] %s", "Error: Can't memory-map video stream file!");
        throw SSRStreamException();
    }
    ...
    for (unsigned int i = 0; i < GLINJECT_RING_BUFFER_SIZE; ++i) {
        ...
        if (fd.m_fd_frame == -1) {
            blog(LOG_ERROR, "[SSRVideoStreamReader::Init] %s", "Error: Can't open video frame file!");
            throw SSRStreamException();
        }
    }
...
void SSRVideoStreamReader::Free() {
    ...
    blog(LOG_INFO, "[SSRVideoStreamReader::Init] %s", "Destroyed video stream reader.");
    ...
    if (required_size > fd.m_mmap_size_frame) {
        ...
        // check frame file size
        {
            ...
            if (fstat(fd.m_fd_frame, &statinfo) == -1 || (size_t) statinfo.st_size < required_size) {
                blog(LOG_ERROR, "[SSRVideoStreamReader::GetFrame] %s", "Error: Size of video frame file is incorrect!");
                throw SSRStreamException();
            }
        ...
        if (fd.m_mmap_ptr_frame == MAP_FAILED) {
            blog(LOG_ERROR, "[SSRVideoStreamReader::GetFrame] %s", "Error: Can't memory-map video frame file!");
            throw SSRStreamException();
        }
