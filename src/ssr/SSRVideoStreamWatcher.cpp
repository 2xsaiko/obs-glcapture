...
void SSRVideoStreamWatcher::Init() {
    ...
    if (mkdir(m_channel_directory.c_str(), (m_relax_permissions) ? 0777 : 0700) == -1) {
        if (errno != EEXIST) { // does the directory already exist?
            blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't create channel directory!");
    ...
    if (lstat(m_channel_directory.c_str(), &statinfo) == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't stat channel directory!");
        throw SSRStreamException();
    }
    if (!S_ISDIR(statinfo.st_mode) || S_ISLNK(statinfo.st_mode)) {
        blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Channel directory is not a regular directory!");
        throw SSRStreamException();
    }
    if (statinfo.st_uid == geteuid()) {
        if (chmod(m_channel_directory.c_str(), (m_relax_permissions) ? 0777 : 0700) == -1) {
            blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't set channel directory mode!");
            throw SSRStreamException();
        }
    } else {
        if (!m_relax_permissions) {
            blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Channel directory is owned by a different user! Choose a different channel name, or enable relaxed file permissions to use it anyway.");
            throw SSRStreamException();
        }
    }
    ...
    if (m_fd_notify == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't initialize inotify!", "don't translate 'inotify'");
        throw SSRStreamException();
    }
    ...
    if (inotify_add_watch(m_fd_notify, m_channel_directory.c_str(), IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO) == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't watch channel directory!");
        throw SSRStreamException();
    }
#endif
    ...
    try {
        ...
        if (dir == NULL) {
            blog(LOG_ERROR, "[SSRVideoStreamWatcher::Init] %s", "Error: Can't open channel directory!");
            throw SSRStreamException();
        }
        ...
        for (;;) {
            ...
            // add the stream
            blog(LOG_INFO, "[SSRVideoStreamWatcher::Init] Added pre-existing stream %s.", d->d_name);
...
void SSRVideoStreamWatcher::HandleChanges(AddCallback add_callback, RemoveCallback remove_callback, void* userdata) {
    ...
    if (ioctl(m_fd_notify, FIONREAD, &len) == -1) {
        blog(LOG_ERROR, "[SSRVideoStreamWatcher::GetChanges] %s", "Error: Can't get read length from inotify!");
        throw SSRStreamException();
    }
    if (len > 0) {
        ...
        if (read(m_fd_notify, buffer.data(), buffer.size()) != (ssize_t) buffer.size()) {
            blog(LOG_ERROR, "[SSRVideoStreamWatcher::GetChanges] %s", "Error: Can't read from inotify!");
            throw SSRStreamException();
        }
        ...
        while (pos < buffer.size()) {
            ...
            if (buffer.size() - pos < sizeof(inotify_event)) {
                blog(LOG_ERROR, "[SSRVideoStreamWatcher::GetChanges] %s", "Error: Received partial event from inotify!");
                throw SSRStreamException();
            }
            ...
            if (buffer.size() - pos < event->len) {
                blog(LOG_ERROR, "[SSRVideoStreamWatcher::GetChanges] %s", "Error: Received partial name from inotify!");
                throw SSRStreamException();
            }
            ...
            if (event->mask & (IN_CREATE | IN_MOVED_TO)) {
                if (std::find(m_streams.begin(), m_streams.end(), stream) == m_streams.end()) {
                    blog(LOG_INFO, "[SSRVideoStreamWatcher::GetChanges] Added stream %s.", stream.m_stream_name.data());
                    m_streams.push_back(stream);
            ...
            if (event->mask & (IN_DELETE | IN_MOVED_FROM)) {
                ...
                if (p != m_streams.end()) {
                    blog(LOG_INFO, "[SSRVideoStreamWatcher::GetChanges] Removed stream %s.", stream.m_stream_name.data());
    ...
    for (unsigned int j = m_streams.size(); j > 0;) {
        ...
        unlink(filename.c_str());
        blog(LOG_INFO, "[SSRVideoStreamWatcher::Init] Deleted abandoned stream %s.", m_streams[j].m_stream_name.data());
